  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 p-8 flex flex-col justify-end">
        ...
        >
          <span className="hidden group-hover:block ui-label text-[10px]">{t('common.exploreJourney')}</span>
          <ArrowRight size={18} />
        </Link>
      </div>
    </motion.div>
    ...
    <section className="py-24 bg-background-light dark:bg-background-dark">
      <div className="container mx-auto px-6">
        <div className="flex justify-between items-end mb-16">
          <div>
            <p className="ui-label text-accent-light dark:text-accent-dark mb-4">{t('common.sacredPlaces')}</p>
            <h2 className="heading-h2">{t('collections.spiritual.title')}</h2>
          </div>
          <Link to="/spiritual" className="hidden md:flex items-center space-x-2 text-muted-light dark:text-muted-dark hover:text-accent-light transition-colors group">
            <span className="ui-label text-[10px]">{t('common.viewAllJourneys')}</span>
