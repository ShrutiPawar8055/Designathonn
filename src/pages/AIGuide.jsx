...
const AIGuide = () => {
  ...
  const [messages, setMessages] = useState([
    { id: 1, type: 'bot', content: 'Namaste. I am your Reverie Guide. Every stone here has a story to tell. What would you like to uncover today?', timestamp: 'Just now' }
  ]);
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      setMessages(prev => [...prev, {
        id: Date.now() + 1,
        type: 'bot',
        content: 'I am processing your request using the ancient archives... (Claude AI Integration pending API key)',
        timestamp: 'Just now'
      }]);
    }, 1000);
  };
          ...
          <aside className="hidden lg:flex lg:w-80 flex-col gap-6">
            <div className="bg-surface-light dark:bg-surface-dark border border-border/10 rounded-[2rem] p-8 shadow-sm">
              <h3 className="ui-label text-xs mb-6 flex items-center gap-2">
                <History size={14} className="text-accent-light" />
                RECENT DISCOVERIES
              </h3>
              <div className="space-y-4">
                {['The architecture of Hampi', 'Varanasi at dusk', 'The legend of Somnath'].map((item, i) => (
                  <button key={i} className="w-full text-left text-xs opacity-60 hover:opacity-100 transition-opacity flex items-center gap-3 group">
                    <div className="w-1 h-1 rounded-full bg-border group-hover:bg-accent-light transition-colors" />
                    {item}
                  </button>
                ))}
              </div>
            ...
            <div className="bg-surface-light dark:bg-surface-dark border border-border/10 rounded-[2rem] p-8 shadow-sm flex-grow">
              <h3 className="ui-label text-xs mb-6 flex items-center gap-2">
                <Settings size={14} className="text-accent-light" />
                GUIDE SETTINGS
              </h3>
              <div className="space-y-6">
                <div>
                  <p className="text-[10px] ui-label opacity-40 mb-3">LANGUAGE</p>
                ...
                <div>
                  <p className="text-[10px] ui-label opacity-40 mb-3">NARRATION STYLE</p>
                  <select className="bg-transparent border-b border-border/20 text-xs py-1 w-full focus:outline-none focus:border-accent-light cursor-pointer">
                    <option>Cinematic Storyteller</option>
                    <option>Academic Historian</option>
          ...
          <main className="flex-grow flex flex-col bg-surface-light dark:bg-surface-dark border border-border/10 rounded-[2.5rem] shadow-2xl overflow-hidden relative">
            ...
            <header className="p-6 md:p-8 border-b border-border/10 flex items-center justify-between bg-heritage-light/5 dark:bg-heritage-dark/5">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-accent-light dark:bg-accent-dark flex items-center justify-center text-white shadow-lg">
                  <Sparkles size={24} />
                </div>
                <div>
                  <h2 className="heading-h3 text-xl">Reverie Intelligence</h2>
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                    <p className="text-[10px] ui-label opacity-40">Ready to converse</p>
                  </div>
                </div>
              </div>
                    ...
                    {messages.map((msg) => (
                      <div key={msg.id} className={`flex ${msg.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[80%] flex gap-4 ${msg.type === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${msg.type === 'user' ? 'bg-heritage-light dark:bg-heritage-dark text-white' : 'bg-accent-light/10 text-accent-light'}`}>
                  ...
                  >
                    <div className="relative mb-12">
                      <motion.div 
                        ...
                        className="absolute inset-0 bg-accent-light/20 rounded-full blur-3xl"
                      />
                      <div className="w-48 h-48 rounded-full border-2 border-accent-light/30 flex items-center justify-center relative bg-background-light dark:bg-background-dark shadow-2xl">
                        <Mic size={48} className="text-accent-light" />
                        <div className="absolute inset-0 flex items-center justify-center">
                          {[1, 2, 3].map((i) => (
                            <motion.div
                              ...
                              className="absolute w-full h-full border border-accent-light/20 rounded-full"
                            />
                          ))}
                        </div>
                      </div>
                    </div>
                    <h3 className="heading-h3 mb-4">Listening for your voice...</h3>
                    <p className="body-text text-sm opacity-60 max-w-xs mb-12">
                      Speak naturally. Ask about legends, history, or the soul of the place you are visiting.
                    </p>
                    <div className="flex gap-4">
                      ...
                      <button className="px-10 py-4 bg-heritage-light dark:bg-heritage-dark text-white rounded-full ui-label text-xs tracking-widest shadow-xl hover:scale-105 transition-all">
                        STOP LISTENING
                      </button>
                      <button className="w-16 h-16 rounded-full border border-border/10 flex items-center justify-center hover:bg-black/5 dark:hover:bg-white/5 transition-all">
            ...
            <footer className="p-6 md:p-8 bg-background-light/30 dark:bg-background-dark/30 backdrop-blur-md border-t border-border/10">
              <div className="relative max-w-4xl mx-auto">
                <input 
                  ...
                  className="w-full bg-surface-light dark:bg-surface-dark border border-border/10 rounded-full px-8 py-5 text-sm focus:outline-none focus:border-accent-light transition-all shadow-xl pr-20"
                />
                <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-2">
                  <button className="w-10 h-10 rounded-full text-muted-light hover:text-accent-light transition-colors flex items-center justify-center">
                    <Mic size={20} />
                  </button>
                  <button 
                    onClick={handleSend}
              ...
              <p className="text-center mt-4 text-[8px] ui-label opacity-30">REVERIE AI CAN MAKE MISTAKES. VERIFY HISTORICAL FACTS IF CRITICAL.</p>
            </footer>
